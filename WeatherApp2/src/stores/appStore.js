...
export const useAppStore = defineStore('app', {
  ...
  getters: {
    weatherIcon:(state) => {
      if (state.weatherdata.value?.weather && state.weatherdata.value.weather.length > 0) {
        const code =state.weatherdata.value.weather[0].id;
        console.log(code)
        if (code == 800) {
    ...
    time: (state) => {
      const seconSinceEpoch=state.weatherdata.value.dt
      console.log(seconSinceEpoch)
      const millisecondsSinceEpoch = seconSinceEpoch * 1000
  ...
  actions: {
    async  getcitycoordinates(){
      try{
        const response = await fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${this.citynameInput}&limit=5&appid=01c3c2368afd7192bc522d6b354bc61d`);
      ...
      await this.fetchweatherdata();
        ...
        alert('getcitycoordinates failed');
      }
    },
    async  fetchweatherdata(){
      try{
        const lat =this.cityobject.lat;
        const lon=this.cityobject.lon;
      ...
      const data = await response.json();
      if (data){
        this.weatherdata.value=data;
        console.log(this.weatherdata.value)
        this.weatherDescription=this.weatherdata.value.weather[0].description;
        this.weatherIconId=this.weatherdata.value.weather[0].id;
        console.log(this.weatherIconId)
        const temp=this.weatherdata.value.main.temp;
         console.log(temp)
         console.log(this.weatherIcon)
        this.temperature = Math.ceil((temp - 273.15) * 10) / 10;
      }
      console.log(this.weatherDescription)
      }
      catch {
        alert('fetchweatherdata failed');
      }
